import { useNavigate } from "react-router-dom"

function MainPage() {
    const navigate = useNavigate()

    const DinePonda = () => {
        navigate('/ponda')
    }

    const DineDhavali = () => {
        navigate('/dhavali')
    }

    return (
        <div className=" w-screen h-screen bg-yellow-400 flex flex-col justify-center items-center">
            <img
                loading="lazy"
                src="/logo.png"
                alt="Chefs Bhojan logo"
                className="object-contain w-1/3 aspect-[1.2] "
            />
            <button onClick={DinePonda} className="flex items-center relative gap-2 px-10 py-5 mt-14 w-2/3 text-sm font-bold text-white bg-red-900 rounded-3xl shadow-[0px_1px_5px_rgba(0,0,0,0.25)] hover:scale-90 transition-transform">Dine in at Ponda
                <img className=" p-2 rounded-lg bg-white absolute right-5" src="data:image/png;base64," />
            </button>
            <button onClick={DineDhavali} className="flex items-center relative gap-2 px-10 py-5 mt-2 w-2/3 text-sm font-bold text-white bg-red-900 rounded-3xl shadow-[0px_1px_5px_rgba(0,0,0,0.25)] hover:scale-90 transition-transform">Dine in at Dhavali
                <img className=" p-2 rounded-lg bg-white absolute right-5" src="data:image/png;base64," />
            </button>
        </div>
    )
}

export default MainPage